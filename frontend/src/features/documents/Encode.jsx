...
const Encode = ({ user }) => {
    ...
    const performUpdateSubmit = async () => {
        ...
        try {
            ...
            if (form.date) fd.append('date', toBackendDateFormat(form.date));
            ...
            if (form.market_period_from) fd.append('market_period_from', toBackendDateFormat(form.market_period_from));
            if (form.market_period_to) fd.append('market_period_to', toBackendDateFormat(form.market_period_to));
            if (form.market_expected_delivery) fd.append('market_expected_delivery', toBackendDateFormat(form.market_expected_delivery));
            ...
            if (form.date_received) fd.append('date_received', toBackendDateFormat(form.date_received));
            ...
            if (form.notarized_date) fd.append('notarized_date', toBackendDateFormat(form.notarized_date));
            }
    ...
    useEffect(() => {
        ...
        // Digital copies no longer show preview automatically.
        // User must click a button to view the file in a new tab.
        const index = Math.min(manageFolderPopupIndex, docs.length - 1);
        setManageFolderPopupPreview({ doc: docs[index], previewBlobUrl: null, previewBlobType: null });
    }, [manageFolderPopup, documents, manageFolderPopupIndex]);
    ...
    const handleChecklistSubDocClick = (subDocWithStatus) => {
        ...
        setForm({
            ...
            notarized_date: formatInputDate(subDocWithStatus.doc.notarized_date) || '',
            ...
            date: formatInputDate(subDocWithStatus.doc.date) || '',
            file: null,
