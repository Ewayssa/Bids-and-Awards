    ...
    return (
        <div className="relative">
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                className="p-2.5 rounded-xl text-[var(--text-muted)] hover:bg-[var(--background-subtle)] hover:text-[var(--text)] transition-all duration-300 ease-out relative group"
            ...
            {open && (
                <>
                    ...
                    <div className="absolute right-0 top-full mt-3 z-[9999] w-80 max-w-[calc(100vw-2rem)] rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300" role="dialog" aria-label="Notifications">
                        <div className="px-5 py-4 border-b border-[var(--border-light)] flex items-center justify-between bg-[var(--background-subtle)]/30">
                            <div>
                            ...
                            {unreadCount > 0 && (
                                <button type="button" onClick={markAllRead} className="text-xs font-semibold text-[var(--primary)] hover:text-[var(--primary-hover)] transition-colors px-2 py-1 rounded-md hover:bg-[var(--primary-muted)]/20">
                                    Mark all read
                                </button>
                            )}
                        </div>
                                    ...
                                    {notifications.map((n) => (
                                        <li key={n.id}>
                                            <button
                                                ...
                                                className={`w-full text-left px-5 py-4 hover:bg-[var(--background-subtle)] transition-all duration-200 group relative ${!n.read ? 'bg-[var(--primary-muted)]/10' : ''}`}
                                            >
                                                {!n.read && (
                        ...
                        <div className="p-3 bg-[var(--background-subtle)]/30 border-t border-[var(--border-light)] text-center">
