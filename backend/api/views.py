...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if doc_by != username.lower():
            # Check full name too
            user = User.objects.filter(username=username).first()
            if not user or doc_by != (user.fullName or '').strip().lower():
                return Response({'detail': 'Unauthorized uploader.'}, status=status.HTTP_403_FORBIDDEN)
